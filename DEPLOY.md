...
Same idea as 1b, but for Adelyn — her full year of Vocabulary and Reading, converted from her own course documents. This one is split across **two files** because the combined content (300KB+) was too large for the Apps Script editor's function dropdown to index reliably — see the troubleshooting note at the end of this doc if you hit that. Only needs to run once, after `setupSheets()` has already populated your Sheet:

1. In the Apps Script editor, add two new script files: `AdelynVocabReadingData` (paste in [`apps-script/AdelynVocabReadingData.gs`](apps-script/AdelynVocabReadingData.gs) — this one is just a big data array, no functions, nothing to run) and `AdelynVocabReadingSeed` (paste in [`apps-script/AdelynVocabReadingSeed.gs`](apps-script/AdelynVocabReadingSeed.gs) — this is the small file with the actual function). Save both.
2. Select **seedAdelynVocabReading_** in the function dropdown (it should appear once you're on the small `AdelynVocabReadingSeed` file) and click **Run**.
...
## Troubleshooting: a seed function's "Run" button/dropdown won't show it

Hit this getting Adelyn's content live (2026-09-04) — worth knowing if it happens again for a future seed file (e.g. Grammar Units 2-5, Writing Weeks 5+):

- **Symptom 1 — function dropdown says "No functions" for a file that clearly has one.** The Apps Script web editor's client-side function-list scanner is unreliable for files that were `clasp push`-ed rather than typed/pasted directly into the editor, and can also just get stuck for reasons that don't obviously correlate with file size (a 5KB file failed after a 300KB one did, in the same session). Things worth trying, roughly in order of effort: reload the whole page (not just click around); close the file's tab and reopen it from the sidebar; delete the file and re-add it via **+ → Script**, pasting content fresh; add a tiny throwaway wrapper function (`function run_() { realFunctionName_(); }`) at the top of the file and see if *that* gets indexed even though the real one doesn't (if so, just run the wrapper — it calls the real function, so it works despite the dropdown bug).
- **Symptom 2 — a function that should take a fraction of a second times out after 6 minutes ("Exceeded maximum execution time").** If the function ends with `SpreadsheetApp.getUi().alert(...)`, that's very likely the cause: `getUi()` has no popup surface to render into when a function is run directly from the script editor (as opposed to from a menu inside the actual Sheet), and instead of failing fast it can hang until Apps Script's hard timeout. **Check the actual Sheet for the expected new rows before assuming nothing happened** — the data write usually already succeeded before the trailing `alert()` call hung. Fix: wrap every `SpreadsheetApp.getUi().alert(msg)` call in `try { SpreadsheetApp.getUi().alert(msg); } catch (e) { Logger.log(msg); }` (already done in all of Adelyn's seed files as of this writing — copy the pattern for any new one).
- **Symptom 3 — a *different*, unrelated function throws a `Cannot read properties of undefined` error the moment you run anything.** This means file *execution order* got scrambled, most likely by a `clasp push` (clasp doesn't preserve the Apps Script editor's original file order by default). Apps Script concatenates every file in the project into one global scope and runs all top-level (non-function-body) code in file order before invoking whatever function you asked for — so if file B's top-level code references a `var` that file A defines, A must run first. Fix: set `"filePushOrder": [...]` in `.clasp.json` (an array of filenames, in the order they should push/execute) and re-push. `Setup.gs` needs to come before anything that references `GRAMMAR_LEVEL_OPTIONS_SEED`, `SHEET_HEADERS`, etc. at its own top level (`Month1Weeks2to5.gs` does).
- **If nothing above works, bypass the dropdown entirely with a temporary Sheet menu.** Add a throwaway file with an `onOpen()` function that builds a custom menu referencing your seed functions **by name as a string** (`.addItem('Seed X', 'seedX_')`), reload the actual Google Sheet (not the script editor) to trigger `onOpen()`, and click the menu items from there. This completely sidesteps the editor's function-scanning bug, and since it's invoked from the real Sheet UI, `SpreadsheetApp.getUi().alert()` also works natively (no hang). Delete/empty the file afterward — `clasp push` does not delete remote-only files even after you delete them locally, so overwrite it with an empty stub rather than assuming a local deletion will remove it from the live project.
